...
use yii\helpers\Url;
use backend\models\AuthManager;
...
class YiiForum
{
    ...
    {
        return \common\models\User::find()->where(['id'=>$userid])->one();
    }
     ...
     */
    public static function uploadFiles($name, $relativeDir='')
    {
        if($relativeDir==''){
            $relativeDir = '/upload/attachment';
        }
        $baseDir = Yii::$app->basePath.'/web'.$relativeDir;
        $model = new UploadsForm();
        $model->file = UploadedFile::getInstancesByName($name);
        if(empty($model->file)){
            return [
                    'errno' => 1,
                    'errmsg' => 'not file upload',
                ];//没有上传文件
        }
        if ($model->validate()) {
            $resFileArr = [];
            foreach ($model->file as $f) {
                ...
                $resFileArr[] = [
                    'fileUrl' => $relativeDir.'/'.$dir.'/'.$fileName,
                    'name' => $f->name,
                ];                
            }
